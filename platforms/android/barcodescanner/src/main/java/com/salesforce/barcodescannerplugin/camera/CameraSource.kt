    ...
    internal fun stop() {
        ...
        processingThread?.let {
            try {
                ...
                Utils.postError(TAG, "Frame processing thread interrupted on stop.", e)
            }
        ...
        camera?.let {
            ...
            try {
                ...
                Utils.postError(TAG, "Failed to clear camera preview", e)
            }
            it.release()
            camera = null
        }
        ...
        override fun run() {
            ...
            while (true) {
                synchronized(lock) {
                    while (active && pendingFrameData == null) {
                        try {
                            ...
                            Utils.postError(TAG, "Frame processing loop terminated.", e)
                            return
                        }
                    }
                ...
                try {
                    ...
                    Utils.postError(TAG, "Exception thrown from receiver.", e)
                } finally {
                    data?.let {
                        camera?.addCallbackBuffer(it.array())
                    }
